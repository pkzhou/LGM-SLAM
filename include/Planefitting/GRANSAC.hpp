	...
	public:
		RANSAC(void)
		{
			...
			for (int i = 0; i < nThreads; ++i)
			{
				...
				m_RandEngines.emplace_back(SeedDevice());
			}
		...
		bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>> &Data, double &inlinerfraction)
		{
			...
			for (int i = 0; i < m_MaxIterations; ++i)
			{
				...
				std::shuffle(RemainderSamples.begin(), RemainderSamples.end(), m_RandEngines[0]); // To avoid picking the same element more than once
			...
			inlinerfraction = m_BestModelScore;
			//std::cout << "BestInliers numbers: " << inliner << std::endl;
