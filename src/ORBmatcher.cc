...
//利用BOW的节点特性，将属于同一节点的ORB特征进行匹配，从而加速匹配
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    DBoW2::FeatureVector::const_iterator KFit = vFeatVecKF.begin();//参考帧的BOW向量开头地址
    DBoW2::FeatureVector::const_iterator Fit = F.mFeatVec.begin();//当前帧的BOW向量开头地址
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first)//步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                if(bestDist1<=TH_LOW)//如果匹配距离小于阈值
                {   
                    // 如果最佳匹配比次佳匹配明显要好，那么最佳匹配才真正靠谱
