...
namespace ORB_SLAM2
{
    static bool groundaid = true;
    static bool groundaid_LOCALBA = true;
    static int weight = 1000;
    static int weight2 = 1000;
    static double weight3 = 2000/weight;
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            else  // Stereo observation 双目
            {
                ...
                //sigma2来自于观测点的偏差，octave (pyramid layer) from which the keypoint has been extracted
                const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
    ...
    if (groundaid) {
        if (pFrame->mbgroundmatch) {
            vector<Eigen::Vector3d> pom;
            pom.resize(4);
            pom[0] << 0.88, 1.5, 1.68;
            pom[1] << -0.72, 1.5, 1.68;
            pom[2] << -0.72, 1.5, -1.03;
            pom[3] << 0.88, 1.5, -1.03;
            KeyFrame* pKFMatch = pFrame->mpMatchKF;
            Eigen::Matrix4d Twj = Converter::toMatrix4d((pFrame->mTcw).inv());
            auto pgp = pKFMatch->mpfittingPlane;
            for (int i = 0; i < 4; i++) {
                double obs = 0.0;
                auto *e = new EdgeGroundProjectOnlyPose();

                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
                e->setMeasurement(obs);

                Eigen::Vector4d pj{pom[i](0,0),pom[i](1,0),pom[i](2,0),1};
                Eigen::Vector4d pw;
                pw = Twj*pj;
                Eigen::Vector3d temp{pw(0,0),pw(1,0),pw(2,0)};
                Eigen::Matrix<double, 1, 1> Sigma = temp.transpose()*(pgp->mMgroundcov)*temp;
                //cout<<" Pose estimation info is "<<Sigma.inverse()<<endl;
                e->setInformation(Sigma.inverse()*weight);
                auto *rk = new g2o::RobustKernelHuber;
                e->setRobustKernel(rk);
                rk->setDelta(deltaStereo);

                e->Tiw = Converter::toSE3Quat(pKFMatch->GetPose());
                e->Po = pom[i];
                auto M = pgp->Getcoefs();
                e->a = M[0];
                e->b = M[1];
...
//    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    ...
    //new definition
    vector<EdgeGroundProjectPlanePose*> vpEdgesPoses;
    vector<EdgeCoplanarPrior*> vpEdgesCoplanar;
    vector<KeyFrame*> vpEdgesKFMatch;
    vector<MapPoint*> vpEdgesMPMatch;
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                ...
                if(pKFi->mvuRight[mit->second]<0)
                {
                    ...
                    g2o::EdgeSE3ProjectXYZ* e = new g2o::EdgeSE3ProjectXYZ();
                    //id是与Keyframe关联的MapPoint的ID，已经被设置为mappoint在POSE的后面
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                ...
                else // Stereo observation
                {
                    ...
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    //cout<<"stereo observation info is"<<invSigma2<<endl;
                    Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2;
                    e->setInformation(Info);
    ...
    {
        KeyFrame* pKFi = *lit;
        if (pKFi->mbgroundflag) {
            auto pgp = pKFi->mpfittingPlane;
            VertexGroundPlane* plane = new VertexGroundPlane();
            plane->setEstimate(pgp->GetcoefsEigen());
            int id = maxKFid + maxMPid + 2 + pgp->mnId;
            if (pgp->mnBAlocalforKF != pKF->mnId) {
                plane->setId(id);
                //plane->setFixed(true);
                optimizer.addVertex(plane);
                pgp->mnBAlocalforKF = pKF->mnId;
            }

        }
    }

        if (groundaid_LOCALBA)
    {
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();lit != lend; lit++)
        {
            KeyFrame *pKFi = *lit;
            if (pKFi->mbgroundmatch) {

                KeyFrame *pKFMatch = pKFi->mpMatchKF;
                vector<Eigen::Vector3d> pom;
                pom.resize(4);
                pom[0] << 0.88, 1.5, 1.68;
                pom[1] << -0.72, 1.5, 1.68;
                pom[2] << -0.72, 1.5, -1.03;
                pom[3] << 0.88, 1.5, -1.03;
                Eigen::Matrix4d Twj = Converter::toMatrix4d((pKFi->GetPose()).inv());
                auto pgp = pKFMatch->mpfittingPlane;

                if (pgp->mnBAlocalforKF == pKF->mnId) {
                    for (int i = 0; i < 4; i++) {
                        double obs = 0.0;
                        auto *e = new EdgeGroundProjectPlanePose();

                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(maxKFid + maxMPid + 2 + pgp->mnId)));
                        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKFi->mnId)));
                        e->setMeasurement(obs);
                        ...
                        Eigen::Matrix<double, 1, 1> Sigma = temp.transpose() * (pgp->mMgroundcov) * temp;
                        //debug
                        //cout<<"pKFMatch->mnId is "<<pKFMatch->mnId<<" pKFi->mnId is "<<pKFi->mnId<<endl;
                        //cout<<"e->a,b,c,d is "<<e->a<<" "<<e->b<<" "<<e->c<<" "<<e->d<<" ";
                        //cout<<"Sigma is "<<Sigma.inverse()<<endl;
                        e->setInformation(Sigma.inverse() * weight);
                        auto *rk = new g2o::RobustKernelHuber;
                        e->setRobustKernel(rk);
                        rk->setDelta(thHuberStereo);
                        ...
                        optimizer.addEdge(e);
                        vpEdgesPoses.push_back(e);
                        vpEdgesKFMatch.push_back(pKFi);
                    }
                }
                else
                {
            ...
            //MapPoints CoPlaner constraints
            if (pKFi->mbgroundflag)
            {
                auto pgp = pKFi->mpfittingPlane;
                ...
                for(list<MapPoint*>::iterator mit=lLocalMapPoints.begin(), mend=lLocalMapPoints.end(); mit!=mend; mit++)
                {
                    MapPoint *pMP = *mit;
                    if (pMP->num>0)
                        if ((pMP->mpMatchKF)->mnId == pKFi->mnId) {
                            double obs = 0.0;
                            auto *e = new EdgeCoplanarPrior();

                            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                    maxKFid + maxMPid + 2 + pgp->mnId)));
                            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                    pMP->mnId+maxKFid+1)));
                            e->setMeasurement(obs);

                            e->setInformation(Eigen::Matrix<double, 1, 1>::Identity() * weight2);
                            auto *rk = new g2o::RobustKernelHuber;
                            e->setRobustKernel(rk);
                            rk->setDelta(thHuberStereo);
                            optimizer.addEdge(e);
                            vpEdgesCoplanar.push_back(e);
                            vpEdgesMPMatch.push_back(pMP);
                        }
                }
            }
        }
    ...
    //cout<<"vpEdgesStereo.size() is "<<vpEdgesStereo.size()<<endl;
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
    ...
    //cout<<"\nvpEdgesPoses.size() is "<<vpEdgesPoses.size()<<endl;
    for(size_t i=0, iend=vpEdgesPoses.size(); i < iend; i++)
    {
        auto e = vpEdgesPoses[i];
        //cout<<"error is:"<<e->chi2()<<",";
        if (e->chi2()>7.815/weight3)
        {
            e->setLevel(1);
        }
    ...
    //cout<<endl;
    //cout<<"vpEdgesCoplanar.size() is "<<vpEdgesCoplanar.size()<<endl;
    for(size_t i=0, iend=vpEdgesCoplanar.size(); i < iend; i++)
    {
        auto e = vpEdgesCoplanar[i];
        //cout<<"error is:"<<e->chi2()<<",";
        if (e->chi2()>7.815/weight3)
        {
            e->setLevel(1);
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
        ...
        for(size_t i=0, iend=vpEdgesPoses.size(); i < iend; i++)
        {
            ...
            if (e->chi2()>7.815/weight3)
            {
                KeyFrame* pKFi = vpEdgesKFMatch[i];
                vToErase2.push_back(pKFi);
            }
        }
        int MPerase = 0;
        for(size_t i=0, iend=vpEdgesCoplanar.size(); i < iend; i++)
        {
            ...
            if (e->chi2()>7.815/weight3)
            {
                MapPoint* pMP = vpEdgesMPMatch[i];
                pMP->num = 0;
                pMP->ground_dist = DBL_MAX ;
                pMP->mpMatchKF = nullptr;
                MPerase++;
            }
    ...
    //remove KFMatch from pKFi
    if(!vToErase2.empty())
    {
        for(size_t i=0;i<vToErase2.size();i++)
        {
            KeyFrame* pKFi = vToErase2[i];
            if (pKFi->mpMatchKF) {
                auto plane = pKFi->mpMatchKF->mpfittingPlane;
                pKFi->mbgroundmatch = false;
                pKFi->mpMatchKF = nullptr;
                int &n = plane->nMatches;
                n--;
            }
        }
    }

    //cout<<"remove number(KFMatch/MP): "<<vToErase2.size()<<", "<<MPerase<<endl;
    // Recover optimized data
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        if (pKF->mbgroundflag)
        {
            auto pgp = pKF->mpfittingPlane;
            array<double,4> coefs = pgp->Getcoefs();
            //cout<<"initialized planeCoefs: "<<coefs[0]<<","<<coefs[1]<<","<<coefs[2]<<","<<coefs[3]<<endl;
            VertexGroundPlane* vPlane = static_cast<VertexGroundPlane*>(optimizer.vertex(maxKFid + maxMPid + 2 + pgp->mnId));
            if (vPlane)
            {
                pgp->Setcoefs(vPlane->estimate());
                coefs = pgp->Getcoefs();
                //cout << "optimized planeCoefs: " << coefs[0] << "," << coefs[1] << "," << coefs[2] << "," << coefs[3]<< endl;
            }
